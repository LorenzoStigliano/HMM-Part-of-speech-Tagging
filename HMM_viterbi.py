...
class HMM:
    ...
    def emission_model(self, train_data):
        ...
        data = [[(x[1], x[0].lower()) if x[1]!="." else (x[1], x[0]) for x in s] for s in train_data]

        emission_FD = nltk.ConditionalFreqDist(list(itertools.chain.from_iterable(data)))

        self.emission_PD = nltk.ConditionalProbDist(emission_FD,HMM.LidProDist)
    ...
    def transition_model(self, train_data):
        ...
        self.transition_PD = nltk.ConditionalProbDist(transition_FD,HMM.LidProDist)
    ...
    def initialise(self, observation):
        ...
        # As for the backpointer data structure, the index at which the pointer lies is
        # the value for the n step.
        # So for example: [3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 11] is the last array in the
        # backpointer so we would calulate the value for the cost which is lowest
        # for the viterbi and get the index say 6. Then the state for the last word
        # would be state[6].
        # Now if we want to find the n-1 word we would index [3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 11]
        # at 6 so the value would be 3 so state[3] would be the state for n-1 and the
        # backpointer would point to backpointer[n-1][3] to find the value for n-2.

        N = len(self.states)

        self.viterbi = [[0]*N]
        for s in range(N):
            self.viterbi[0][s] = -(self.tlprob("<s>",self.states[s])+self.elprob(self.states[s],observation))

        # Initialise step 0 of backpointer
        self.backpointer = [[0]*N]
    ...
    def tag(self, observations):
        ...
        # Create all the remaining entries in the 2D arrays and set them all equal to 0.
        for i in range(T-1):
            self.viterbi.append([0]*N)
            self.backpointer.append([0]*N)

        for t in range(T): #iterate over steps
            if(t != 0):
                for s in self.states: #iterate over states
                    # Find min over all states viterbi[T-1][s']-a(s',s)-b(s,Ot))
                    # Find argmin for the back pointer
                    # NOTE :a = tlprob b = elprob
                    min_cost = self.viterbi[t-1][0]-(self.tlprob(self.states[0],s)+self.elprob(s,observations[t]))
                    pointer = 0
                    for s_prime in range(1,N):
                        cost = self.viterbi[t-1][s_prime]-(self.tlprob(self.states[s_prime],s)+self.elprob(s,observations[t]))
                        if cost < min_cost:
                            min_cost = cost
                            pointer = s_prime

                    # Viterbi at step t for state s
                    self.viterbi[t][self.states.index(s)] = min_cost
                    # Backpointer at step t for state s
                    self.backpointer[t][self.states.index(s)] = pointer
        ...
        for i in range(T,-1,-1):
            if(i!=1 and i!=0):
                #Get the last state
                next_state = self.states[self.backpointer[i-1][last_pointer]]
                tags.append(next_state)
                #Update the last_pointer to be the value for the next word
                last_pointer = self.backpointer[i-1][last_pointer]
    ...
    def get_viterbi_value(self, state, step):
        ...
        :rtype: float
        """
        # Since we are using a 2D array it can only be acessed by ints so we have
        # to convert the state given to the index in the self.state list.
...
def answer_question4b():
    ...
    The tagger thought that "racing" was an ADJ but it is a VERB and 'but' CONJ
...
def answer_question5():
    ...
    To ensure that the grammar produces any well-formed sentence,we use the POS
...
def answer_question6():
    ...
    """
    return inspect.cleandoc("""\
    To get better estimates for the emission and transition probabilities.
...
def answers():
    ...
    for sentence in test_data_universal:
        ...
        if not(correct_words == len(sentence))  and count <10:
            count +=1
            print("Correct sentence:")
            print(sentence)
            print("Tagged sentence by HMM:")
